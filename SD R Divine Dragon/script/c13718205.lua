...
function c13718205.initial_effect(c)
	...
	e1:SetCondition(c13718205.spdcon)
	e1:SetTarget(c13718205.spdtg)
	e1:SetOperation(c13718205.spdop)
	c:RegisterEffect(e1)
	--spsummon
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetCode(EVENT_TO_GRAVE)
	e2:SetOperation(c13718205.regop)
	c:RegisterEffect(e2)
	...
	e3:SetCountLimit(1)
	e3:SetCondition(c13718205.spcon)
	e3:SetTarget(c13718205.sptg)
	e3:SetOperation(c13718205.spop)
	c:RegisterEffect(e3)
end
...
function c13718205.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c13718205.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c13718205.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
