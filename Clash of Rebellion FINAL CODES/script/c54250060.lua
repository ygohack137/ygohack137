...
function c54250060.initial_effect(c)
	...
	e4:SetCost(c54250060.cost)
	e4:SetTarget(c54250060.target)
	e4:SetOperation(c54250060.activate)
	c:RegisterEffect(e4)
end
function c54250060.tg(e,c)
	return c:IsRace(RACE_DRAGON+RACE_WARRIOR+RACE_SPELLCASTER)
end
function c54250060.costfilter(c)
	return c:IsDiscardable()
end
function c54250060.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c54250060.costfilter,tp,LOCATION_HAND,0,1,e:GetHandler()) end
...
function c54250060.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c54250060.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c54250060.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c54250060.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
	if tc then
...
function c54250060.rfilter(c)
	return c:IsRace(RACE_SPELLCASTER+RACE_WARRIOR) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost()
end
function c54250060.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c54250060.rfilter,tp,LOCATION_GRAVE,0,4,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c54250060.rfilter,tp,LOCATION_GRAVE,0,4,4,nil)
...
function c54250060.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c54250060.afilter,tp,LOCATION_DECK,0,1,1,nil)
