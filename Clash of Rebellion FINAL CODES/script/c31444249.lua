...
function c31444249.initial_effect(c)
	...
	e3:SetValue(c31444249.val)
	c:RegisterEffect(e3)
	--Activate
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_SZONE)
	e4:SetCost(c31444249.cost)
	e4:SetTarget(c31444249.target)
	e4:SetOperation(c31444249.activate)
	c:RegisterEffect(e4)
end
function c31444249.lvtg(e,c)
	return c:IsSetCard(0xbb) and c:GetOriginalLevel()>=2
end
function c31444249.val(e,re,dam,r,rp,rc)
	if bit.band(r,REASON_BATTLE)~=0 and (rc and rc:IsSetCard(0xbb)) then
		return dam/2
	else return dam end
end

function c31444249.cfilter(c)
...
function c31444249.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToGraveAsCost() end
	Duel.SendtoGrave(e:GetHandler(),REASON_COST)
end
function c31444249.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
	Duel.SendtoGrave(e:GetHandler(),REASON_COST)
		local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
		local mg1=Duel.GetMatchingGroup(c31444249.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
		if Duel.IsExistingMatchingCard(c31444249.cfilter,tp,0,LOCATION_MZONE,1,nil) and not 
		   Duel.IsExistingMatchingCard(c31444249.cfilter,tp,LOCATION_MZONE,0,1,nil) then
...
function c31444249.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	local mg1=Duel.GetMatchingGroup(c31444249.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
	if Duel.IsExistingMatchingCard(c31444249.cfilter,tp,0,LOCATION_MZONE,1,nil) then
		local sg=Duel.GetMatchingGroup(c31444249.filter1,tp,LOCATION_DECK,0,nil,e)
		mg1:Merge(sg)
	end
