...
function c92693205.initial_effect(c)
	...
	e1:SetTarget(c92693205.target)
	e1:SetOperation(c92693205.operation)
	c:RegisterEffect(e1)
	--destroy
...
function c92693205.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c92693205.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c92693205.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsDestructable()
end
function c92693205.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c92693205.cfilter,tp,LOCATION_MZONE,0,1,e:GetHandler())
		and Duel.IsExistingMatchingCard(c92693205.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	local g=Duel.GetMatchingGroup(c92693205.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
