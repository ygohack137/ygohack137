...
function c93983867.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummonStep(tc,0,tp,1-tp,true,false,POS_FACEUP) then
		...
		e1:SetCondition(c93983867.ctcon)
		e1:SetOperation(c93983867.ctop)
		e1:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e1,tp)
		tc:RegisterFlagEffect(93983867,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1,fid)
	end
	Duel.SpecialSummonComplete()
end
function c93983867.ctcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	if not tc:GetFlagEffectLabel(93983867)==e:GetLabel() then
		e:Reset()
		return false
	else return true end
end
function c93983867.ctop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
