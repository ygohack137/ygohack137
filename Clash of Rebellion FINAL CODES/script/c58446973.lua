...
function c58446973.tgfilter(c)
	return c:IsSetCard(0xbb) and not c:IsCode(58446973) and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
function c58446973.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c58446973.tgfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_DECK)
end
function c58446973.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c58446973.tgfilter,tp,LOCATION_DECK,0,1,1,nil)
	local tc=g:GetFirst()
