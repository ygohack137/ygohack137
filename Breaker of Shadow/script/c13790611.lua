...
function c13790611.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c13790611.cfilter,tp,LOCATION_REMOVED,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c13790611.cfilter,tp,LOCATION_REMOVED,0,1,1,nil,tp)
	local atk=g:GetFirst():GetAttack()
	e:SetLabel(atk)
	Duel.SendtoDeck(g,nil,2,REASON_EFFECT)
end
function c13790611.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(c13790611.dfilter,tp,0,LOCATION_MZONE,1,nil,e:GetLabel()) end
	local g=Duel.GetMatchingGroup(c13790611.dfilter,tp,0,LOCATION_MZONE,nil,e:GetLabel())
