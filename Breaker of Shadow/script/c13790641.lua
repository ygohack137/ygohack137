...
function c13790641.initial_effect(c)
	...
	e1:SetOperation(c13790641.regop)
	c:RegisterEffect(e1)
	...
	e2:SetTarget(c13790641.ztg)
	e2:SetOperation(c13790641.zop2)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	c:RegisterEffect(e3)
end
function c13790641.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTarget(c13790641.ztg)
	e1:SetOperation(c13790641.zop)
	e1:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
	c:RegisterEffect(e1)
end
function c13790641.ztg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c13790641.zop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCode(EFFECT_DISABLE_FIELD)
	e1:SetOperation(c13790641.disop)
	e1:SetReset(RESET_EVENT+0x1ff0000)
	e1:SetLabel(e:GetLabel())
	e:GetHandler():RegisterEffect(e1)
end
function c13790641.zop2(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetType(EFFECT_TYPE_FIELD)
