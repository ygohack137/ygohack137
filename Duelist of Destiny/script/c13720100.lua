...
function c13720100.initial_effect(c)
	...
	e1:SetCost(c13720100.cost)
	e1:SetTarget(c13720100.target)
	e1:SetOperation(c13720100.activate)
	c:RegisterEffect(e1)
end
function c13720100.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,nil,2,nil) end
	local sg=Duel.SelectReleaseGroup(tp,nil,2,2,nil)
	Duel.Release(sg,REASON_COST)
end
function c13720100.filter(c,e)
	return c:IsFaceup() and c:IsControlerCanBeChanged()
end
function c13720100.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
